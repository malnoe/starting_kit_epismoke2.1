...
author: "Garance Malnoë,  Léonie Breuza, Matthias Mazet"
date: "`r Sys.Date()`"
output:
  pdf_document:
    toc: true
...
## ***XGBoost***
### ***Idée du modèle***
XGBoost est un modèle de machine learning s'appuyant sur les arbres de décision auxquels est rajouté un boosting de gradient. Il est très rapide et efficace, notamment sur les grands jeux de données. En effet, il s'appuie sur le calcul parallèle et distribué. 

Cependant, il est moins optimal lorsque le jeu de données a une proportion de dimensions plus élévée que celle du nombre d'individus. En effet, les modèles d'arbres vont avoir tendance à sur-ajuster sur des ensembles de données plus petits. Ce qui est notre cas ici.
Il peut être modulé selon un grand nombre d'hyperparamètres, tels que : 

- nrounds : nombre d'arbres à construire.

- max_depth : profondeur maximale des arbres.

- colsample_bytree : fraction des colonnes utilisée pour chaque arbre.

- eta : taux d'apprentissage du modèle.

- gamma : réduction minimale de la perte nécessaire pour diviser un noeud.

- min_child_weight : poids minimum nécessaire pour un noeud.

- subsample : fractionnement des données d'entraînement utilisé pour chaque arbre.

### ***Premier modèle***
Pour commencer, nous séparons le jeu de données d'entraînement en 2 jeux de données :
```{r}
set.seed(1) # Pour avoir des résultats aléatoire reproductibles
...
Ensuite, nous le structurons selon les éléments nécessaires à la construction du modèle :
```{r}
X_train <- as.matrix(data_train1[,5:ncol(data_train1)])
...
Nous créons un objet permettant de faciliter l'optimisation des paramètres au sein du modèle, à l'aide la validation croisée.
```{r}
xgb_trcontrol <- trainControl(method = "cv", number = 5, allowParallel = TRUE, verboseIter = FALSE, returnData = FALSE)
```

Nous créons une grille avec les séquences des hyperparamètres à tester, afin de toruver les meilleures pour la prédiction. 
```{r}
xgbGrid <- expand.grid(nrounds = seq(10, 1000, by = 50),  
                       max_depth = seq(1, 10, by = 1),
                       colsample_bytree = seq(0.01, 1, by = 0.1),
...
Nous développons ensuite une barre de progression, associée au calcul du modèle, afin de pouvoir visualiser l'avancement. 
```{r}
# Création de la barre de progression
total_iters <- length(xgbGrid)  # Nombre total d'itérations (selon le nombre d'hyperparamètres dans tuneGrid)

# Fonction de calcul du modèle
train_with_progress <- function(X_train, Y_train, xgb_trcontrol, tuneGrid) {
  model <- NULL
  for (i in 1:total_iters) {
    
    # Entraînement du modèle pour la combinaison d'hyperparamètres en cours
    model <- train(x = X_train, Y_train, method = "xgbTree",  
                   trControl = xgb_trcontrol, tuneGrid = tuneGrid[i, , drop = FALSE], metric = "Accuracy")
  }
  return(model)
}
```
...
xgb_model <- train_with_progress(X_train, Y_train, xgb_trcontrol, xgbGrid)

# Meilleurs paramètres du modèle
meilleur_param <- xgb_model$bestTune
meilleur_param

# Prédictions du jeu de données test
predict_xgb <- predict(xgb_model, X_test)
predict_prob_xgb <- predict(xgb_model, X_test, type = "prob")

# Matrice de confusion
confusionMatrix(predict_xgb, Y_test)

# Calcul taux de précision et d'erreur
tx_prec_xgb <- sum(predict_xgb == Y_test)/length(Y_test)
tx_prec_xgb
tx_erreur_xgb <- 1 - tx_prec_xgb
tx_erreur_xgb
```

Le taux d'erreur est très important avec ce modèle, proche de celui du modèle null : `r tx_erreur_xgb`. 
Ce modèle n'est donc actuellement pas performant pour la prédiction. Cela peut être dû au fléau des dimensions. 

### ***Modèle avec réduction des dimensions à l'aide d'une ACP***
Pour lutter contre le problème précédent, nous avons effectué une ACP sur les jeux de données, afin d'en réduire les dimensions.
```{r}
X_train_acp <- PCA(X = X_train, graph = FALSE, scale.unit = TRUE)$ind$coord
X_test_acp <- PCA(X = X_test, graph = FALSE, scale.unit = TRUE)$ind$coord
```

Afin de pouvoir les comparer, nous avons laissé la même liste d'hyperparamètres. 
```{r}
# Entraînement du modèle avec la barre de progression
xgb_model_acp <- train_with_progress(X_train_acp, Y_train, xgb_trcontrol, xgbGrid)

# Meilleurs paramètres du modèle
meilleur_param_acp <- xgb_model_acp$bestTune
meilleur_param_acp

# Prédictions du jeu de données test
predict_xgb_acp <- predict(xgb_model_acp, X_test_acp)
predict_prob_xgb_acp <- predict(xgb_model_acp, X_test_acp, type = "prob")

# Matrice de confusion
confusionMatrix(predict_xgb_acp, Y_test)

# Calcul taux de précision et d'erreur
tx_prec_xgb_acp <- sum(predict_xgb_acp == Y_test)/length(Y_test)
tx_prec_xgb_acp
tx_erreur_xgb_acp <- 1 - tx_prec_xgb_acp
tx_erreur_xgb_acp
```

Encore une fois, le taux d'erreur est trop important est quasimment identique au précédent : `r tx_erreur_xgb_acp`. Ce modèle ne semble donc pas pertinent pour l'analyse de ce jeu de données. 

\newpage

## ***SVM***
### ***Idée du modèle***
Afin de lutter contre les problèmes du XGBoost, nous nous sommes renseignés sur les méthodes de classification pertinentes sur les jeux de données ayant plus de variables que d'individus. La SVM semble correspondre à cette problématique. 

La SVM se base sur un hyperplan où les données sont séparées en plusieures classes, afin de répondre à des problèmes de classification. 

Il existe plusieurs hyperparamètres pouvant être modulés afin d'améliorer le modèle : 

- kernel : le type de noyau à utiliser.

- C : le paramètre de régularisation de précision.

- degree : le degré du polynôme si le noyau est de type polynomial.

- probability : le vecteur des probabilités d'apartenir à chaque classe de la variable réponse.

- gamma : l'influence d'un seul exemple d'entraînement si le noyau est de type radial.

### ***Premier modèle***
Dans un premier temps, nous séparons toujours le jeu de données d'entraînement en 2 jeux de données. Et nous enlevons les colonnes contenant la variable d'intérêt sous la forme de 3 variables binaires.
```{r}
set.seed(1) # Pour avoir des résultats aléatoire reproductibles
...
Nous structurons les données sous la forme qui nous intéresse pour ce modèle :
```{r}
X_train <- as.matrix(data_train1[,5:ncol(data_train1)])
Y_train <- data_train1[,1]
Y_train <- as.factor(Y_train)
...
Nous testons ensuite le modèle avec le noyau par défaut, c'est-à-dire radial.
```{r}
svm_modele_rad <- svm(smoking_status ~ ., 
                  data = data_train1, kernel = "radial" )

```

Nous effectuons ensuite les prédictions : 
```{r}
predict_svm_rad <- predict(svm_modele_rad, data_train2[, -1])

as.vector(predict_svm_rad)
```

Puis, nous observons la matrice confusion, le taux de précision et d'erreur.
```{r}
confusionMatrix(predict_svm_rad, data_train2$smoking_status)

tx_prec_rad <- sum(predict_svm_rad == Y_test)/length(Y_test)
tx_prec_rad
tx_erreur_rad <- 1 - tx_prec_rad
tx_erreur_rad
```

Nous pouvons que le taux d'erreur a nettement diminué par rapport au XGBoost : `r tx_erreur_rad`. Il faut maintenant essayer avec une réduction des dimensions afin de comparer.

### ***Modèle avec réduction des dimensions à l'aide d'une ACP***
Nous commençons par calculer les coordonnées des individus avec l'ACP. Puis, nous les restrucuturons pour pouvoir calculer le modèle.
```{r}
...
Nous calculons le modèle, toujours avec le noyau par défaut ra
```{r}
svm_modele_acp <- svm(Y_train ~ ., 
                  data = data_train_acp, kernel = "linear" )
```

Nous prédisons ensuite les données du jeu de données test. 
```{r}
predict_svm_acp <- predict(svm_modele_acp, data_test_acp[, -1])
```

Nous calculons de nouveau la matrice de confusion, le taux de précision et le taux d'erreur.
```{r}
...
L'ACP ne réduit pas le taux d'erreur : `r tx_erreur_acp`. Donc nous pouvons conserver le modèle sans effectuer l'ACP.
Nous allons maintenant essayer le premier modèle avec des noyaux différents. Cette étape pourrait se faire par validation croisée, mais ici nous l'avons effectuée de manière expérimentale. C'est donc une amélioration à apporter à notre code.

### ***Modèle avec choix du noyau***
Nous testons ensuite le modèle avec le noyau par défaut, c'est-à-dire radial.
```{r}
svm_modele_p <- svm(smoking_status ~ ., 
                  data = data_train1, kernel = "polynomial" )
svm_modele_s <- svm(smoking_status ~ ., 
                  data = data_train1, kernel = "sigmoid" )
svm_modele_l <- svm(smoking_status ~ ., 
                  data = data_train1, kernel = "linear" )
```

Nous effectuons ensuite les prédictions : 
```{r}
predict_svm_p <- predict(svm_modele_p, data_train2[, -1])
as.vector(predict_svm_p)

predict_svm_s <- predict(svm_modele_s, data_train2[, -1])
as.vector(predict_svm_s)

predict_svm_l <- predict(svm_modele_l, data_train2[, -1])
as.vector(predict_svm_l)
```

Puis, nous observons le taux de précision et d'erreur.
```{r}
tx_prec_p <- sum(predict_svm_p == Y_test)/length(Y_test)
tx_prec_p
tx_erreur_p <- 1 - tx_prec_p
tx_erreur_p

tx_prec_s <- sum(predict_svm_s == Y_test)/length(Y_test)
tx_prec_s
tx_erreur_s <- 1 - tx_prec_s
tx_erreur_s

tx_prec_l <- sum(predict_svm_l == Y_test)/length(Y_test)
tx_prec_l
tx_erreur_l <- 1 - tx_prec_l
tx_erreur_l
```

Comme nous pouvons l'observer, le modèle SVM utilisant le noyau linéaire est le plus performant avec un taux d'erreur de `r tx_erreur_l`.
Afin de l'améliorer, il faudrait essayer de valider le paramètre du noyau avec de la validation croisée, de même pour les hyperparamètres cités au début.
...
tx_erreur_xgb_acp
```
Le taux d'erreur avec le modèle final de XGBoost est de : `r tx_erreur_xgb_acp`. 

### **Résultats sur data_test**
Les résultats étant vraiment mauvais sur le jeu de données train2, nous ne l'avons donc pas soumis. De plus, Léonie n'avait pas de compte valide sur Codabench. 
...
# Prédiction des individus tests
predict_svm_l <- predict(svm_modele_l, data_train2[, -1])
as.vector(predict_svm_l)
...
### **Résultats sur data_test**
La soumission correspondante sur Codabench est la numéro 603 du 18 décembre 2024 publiée par l'utilisateur mazetma car Léonie n'avait pas de compte valide. Le résultat obtenu est de 0,275. Il a été obtenu avec le programme suivant :
...
program <- function(data_train, data_test) {
    ...
    # Restructuration des fichiers d'entraînement et de test
    data_train <- data_train[, -c(2:4)]
    data_test <- data_test[, -c(1:4)]
  
    # Calcul du modèle SVM avec noyeau de type linéaire
    svm_modele <- svm(smoking_status ~ ., 
                    data = data_train, kernel = "linear" )
  
    # Prédictions des individus tests
    data_pred <- predict(svm_modele, data_test)
    data_pred <- as.vector(data_pred)
...
Pour la méthode XGBoost, nous n'avons pas testé sur le jeu de données `data_test` à la vue de ses performances médiocres sur `data_train2`.

Pour la méthode SVM, il y a un tout petit décalage entre le taux d'erreur sur `data_train2` et `data_test`. A nouveau, cette différence est sûrement plutôt liée à de légères différences entre les jeux de données plutôt que du sur-apprentissage.

## ***Proposer des faits pour détailler les résultats (cohérence ou différences)***
Le premier modèle présente des performances modestes, avec un taux d'erreur supérieur à 30 %. Cela peut s'expliquer par sa simplicité : le tri des sondes repose uniquement sur la méthode SIS suivi d'un STEP, ce qui laisse probablement subsister des variables corrélées, ce qui amène du bruit dans le modèle. Par ailleurs, les modèles linéaires simples ne semblent pas parfaitement adaptés à notre problématique, caractérisée par un très grand nombre de variables (~1000) pour un nombre relativement restreint d'individus (~600). En revanche, le modèle Elastic Net, choisi spécifiquement pour répondre à ces limitations, s'avère plus performant, avec un taux d'erreur inférieur à 30 %. Et plus encore, le modèle SVM qui est adaptés aux jeux de données de grande dimensions montre encore des résultats plus performants. Cette dernière méthode pourrait même sans doute être encore améliorée avec du tunning de tous les hyperparamètres qui n'a pas été effectuée ici car elle nécessite encore d'autres fonctions et d'autres packages.
