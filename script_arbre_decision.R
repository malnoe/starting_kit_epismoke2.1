...
i=8
abline(v=i, col=2)
# Commentaire : On voit qu'il y a un sur apprentissage après les 8 premières sondes : on va faire un step pour choisir les meilleurs sondes à parir du critère d'Aikaike (AIC).
# On effectue un step 
# Hyperparamètre : le nombre de probes à prendre en compte
...
# Test de l'accuracy sur le step_model
# Hyperparamètre : le seuil
...
i=8
abline(v=i, col=2)
# Commentaire : On voit qu'il y a un sur apprentissage après les 8 premières sondes : on va faire un step.
# On effectue un step 
...
# Mix and match et hyperparmétrage

# Commentaire : On crée tous les modèles en une fois pour éviter d'avoir à les recalculer à chaque fois (ce qui peut prendre beaucoup de temps parfois)
# Modèles pour current
...
print("NEVER CURRENT")
hyperparametrage_never_current(data_train2,modeles_never,list(0.45,0.475,0.5,0.525,0.55),modeles_current,list(0.45,0.475,0.5,0.525,0.55))
...
print("NEVER FORMER")
hyperparametrage_never_former(data_train2,modeles_never,list(0.45,0.475,0.5,0.525,0.55),modeles_former,list(0.45,0.475,0.5,0.525,0.55))
...
hyperparametrage_former_current(data_train2,modeles_former,list(0.45,0.475,0.5,0.525,0.55),modeles_current,list(0.45,0.475,0.5,0.525,0.55))
...
print("FORMER_NEVER")
hyperparametrage_former_never(data_train2,modeles_former,list(0.45,0.475,0.5,0.525,0.55),modeles_never,list(0.45,0.475,0.5,0.525,0.55))

# Resultats :
# CURRENT NEVER : 0.60 10  0.45  4  0.45
# CURRENT FORMER : 
# NEVER CURRENT : 
# NEVER FORMER : 
# FORMER CURRENT : 
