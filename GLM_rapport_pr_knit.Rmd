...
# Mettre toutes les librairies ici comme ça on se perd pas (on mettra le bloc au début du rapport après)
library(knitr)
library(Matrix)
library(glmnet)
...
- On parcours les valeurs possible de $\alpha$, on construit le modèle Elastic Net associé à chacune et on stock la précision des ` length(alpha)` modèles dans `accuracy_int`.
...
```
Le taux d'erreur avec le modèle final d'Elastic Net est ` 1 - accuracy`.
